...
const SalesPage = () => {
  ...
  useEffect(() => {
    ...
    fetch("/api/Sales", { signal: abortController.signal })
      .then((response) => {
        if (!response.ok) {
  ...
  const fetchItemPrices = async (abortController: AbortController) => {
    try {
      const response = await fetch("/api/Items", {signal: abortController.signal});
      if (!response.ok) {
